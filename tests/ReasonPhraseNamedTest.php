 ...
 */
class ReasonPhraseNamedTest extends TestCase
{
    /**
     * @var ReasonPhrase[]
     */
    ...
    {
        $reasonPhrase = ReasonPhrase::tryFromName(name: null);
        Assert::assertThat($reasonPhrase, Assert::isNull());
    }

    public function testNullFromInteger(): void
    {
        $reasonPhrase = ReasonPhrase::tryFromInteger(integer: null);
        Assert::assertThat($reasonPhrase, Assert::isNull());
    }
    ...
    {
        $invalidName = 'INVALID';
        $reasonPhrase = ReasonPhrase::tryFromName(name: $invalidName);
        Assert::assertThat($reasonPhrase, Assert::isNull());
        $this->expectException(ValueError::class);
        ReasonPhrase::fromName(name: $invalidName);
    }

    public function testInvalidFromInteger(): void
    {
        $invalidValue = -1;
        $reasonPhrase = ReasonPhrase::tryFromInteger(integer: $invalidValue);
        Assert::assertThat($reasonPhrase, Assert::isNull());
        $this->expectException(ValueError::class);
        ReasonPhrase::fromInteger(integer: $invalidValue);
    }
    ...
    public function testCode306Name(): void
    {
        ...
        Assert::assertThat($reasonPhrase, Assert::isNull());
        $this->expectException(ValueError::class);
        ReasonPhrase::fromName(name: $name);
    }

    public function testCode306Integer(): void
    {
        $int306 = 306;
        $reasonPhrase = ReasonPhrase::tryFromInteger(integer: $int306);
        Assert::assertThat($reasonPhrase, Assert::isNull());
        $this->expectException(ValueError::class);
        ReasonPhrase::fromInteger(integer: $int306);
    }
}
